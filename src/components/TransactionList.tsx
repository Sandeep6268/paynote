...
import { ArrowUpRight, ArrowDownRight, Calendar, Edit, Trash2, AlertTriangle, X } from "lucide-react";
          ...
          <CardContent className="p-6 space-y-4">
            <div className="text-gray-300 space-y-3">
              ...
              <div className="bg-gray-700/50 rounded-lg p-3 border border-gray-600">
                <p className="font-semibold text-white">{deleteDialog.transaction?.personName}</p>
                <p className={`text-sm font-medium ${deleteDialog.transaction?.type === 'given' ? 'text-red-400' : 'text-green-400'}`}>
                  {deleteDialog.transaction?.type === 'given' ? 'Payment Given' : 'Payment Received'} - ₹{deleteDialog.transaction?.amount.toLocaleString()}
                </p>
              </div>
